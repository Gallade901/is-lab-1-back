    ...
    public void initialize() {
        try {
            ...
            createFunction("CREATE OR REPLACE FUNCTION delete_one_by_view(view_value TEXT) RETURNS VOID AS $$ " +
                    "BEGIN " +
                    "    DELETE FROM flat WHERE view = view_value AND id IN (SELECT id FROM flat WHERE view = view_value LIMIT 1); " +
                    "END; $$ LANGUAGE plpgsql;");

            createFunction("CREATE OR REPLACE FUNCTION get_flat_by_name_prefix(prefix TEXT) RETURNS SETOF flat AS $$ " +
                    "BEGIN RETURN QUERY SELECT * FROM flat WHERE name LIKE prefix || '%'; END; $$ LANGUAGE plpgsql;");

            createFunction("CREATE OR REPLACE FUNCTION get_cheapest_balcony_flat() RETURNS SETOF flat AS $$ " +
                    "BEGIN RETURN QUERY SELECT * FROM flat WHERE balcony = TRUE ORDER BY price ASC LIMIT 1; END; $$ LANGUAGE plpgsql;");

            createFunction("CREATE OR REPLACE FUNCTION get_sorted_flat_by_metro_time() RETURNS SETOF flat AS $$ " +
